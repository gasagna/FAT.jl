...
OpenFoamToHDF5(casedir; fielddtype=Float64, overwrite=true, dimensions=2)

# # try to write it again
# @test_throws ErrorException OpenFoamToHDF5(casedir)

# # try different dimensions
# @test_throws ErrorException OpenFoamToHDF5(casedir; overwrite=true, dimensions=1)
# @test_throws ErrorException OpenFoamToHDF5(casedir; overwrite=true, dimensions=4)

# # test type of saved data
# for dtype in [Float32, Float64]
# 	OpenFoamToHDF5(casedir; fielddtype=dtype, overwrite=true, dimensions=2)
# 	fh = h5open("$casedir/data.h5")
# 	@test eltype(read(fh["1.000000/internalField/U"])) == dtype
# 	# @test eltype(read(fh["1.000000/boundaryField/U"])) == dtype
# 	close(fh)
# end

# # check attribute for dimension
# for D in ["2", "3"]
# 	OpenFoamToHDF5(casedir; overwrite=true, dimensions=parse(Int, D))
# 	fh = h5open("$casedir/data.h5")
