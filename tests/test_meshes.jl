...
folder. In that folder, all heavy data is expressed in binary format, so that it is
faster to read. 

In this file we test the mesh functionality on both the ascii and 
binary versions. The tests are defined based on the data that we have in the
ascii files, but they should also work for the binary data, out of the box, 
with no errors.

=# 
for casedir in ["./ldc_test_binary", "./ldc_test_ascii"]
    m = Mesh(casedir)
