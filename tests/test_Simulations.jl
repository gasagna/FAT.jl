...
# use a test run
sim = SimulationData(casedir)

# test number of snapshots
@test length(sim) == 10
@test length(times(sim)) == 10

# check times. Note we dropped the initial condition at zero
@test sim.t == collect(1:10)
@test times(sim) == collect(1:10)

# check dimensions of the field
@test ndims(sim) == 2

# check we return a mesh
@test typeof(mesh(sim)) == HexaMesh{Float64}
