...
b = zero(a)
@test sum(abs2, b.internalField) == 0
@test sum(abs2, b.boundaryField) == 0

a = VectorField(mesh(sim), 3)
b = zero(a)
for d = 1:3
    @test sum(abs2, b.scalars[d].internalField) == 0
    @test sum(abs2, b.scalars[d].boundaryField) == 0
end

a = TensorField(mesh(sim), 3)
b = zero(a)
for di = 1:3, dj = 1:3
    @test sum(abs2, b.vectors[dj].scalars[di].internalField) == 0
    @test sum(abs2, b.vectors[dj].scalars[di].boundaryField) == 0
end
...
# subtraction
u = vfields(sim, :U, 1:10)[1].scalars[1]
out = u - u
